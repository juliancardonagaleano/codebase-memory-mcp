...
static char *node_text(JavaLSPContext *ctx, TSNode node);
...
const CBMType *java_parse_type_node(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "type_parameter") == 0) {
        ...
        char *name = ts_node_is_null(name_node) ? NULL : node_text(ctx, name_node);
...
static const CBMType *eval_field_access(JavaLSPContext *ctx, TSNode node) {
    ...
    char *fname = node_text(ctx, field);
...
static const CBMType *return_type_of(const CBMRegisteredFunc *f) {
...
static const CBMType *eval_method_invocation(JavaLSPContext *ctx, TSNode node) {
    ...
    char *mname = node_text(ctx, name_node);
    ...
    if (base && base->kind == CBM_TYPE_TEMPLATE) {
        ...
        if (base_qn) {
            ...
            if (f) {
                const CBMType *ret = return_type_of(f);
...
void java_process_statement(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "local_variable_declaration") == 0) {
        ...
        if (!ts_node_is_null(formal)) {
            ...
            char *pn = ts_node_is_null(pname) ? NULL : node_text(ctx, pname);
            if (pn) {
...
static void process_local_var_decl(JavaLSPContext *ctx, TSNode node) {
    ...
    bool is_var =
        ...
        ({
            char *txt = node_text(ctx, type_node);
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        char *vname = node_text(ctx, name_node);
        if (!vname) continue;
...
static void process_enhanced_for(JavaLSPContext *ctx, TSNode node) {
    ...
    char *vname = ts_node_is_null(name_node) ? NULL : node_text(ctx, name_node);
    if (!vname) return;
    ...
    if (!ts_node_is_null(type_node) &&
        ...
        char *tt = node_text(ctx, type_node);
        if (tt && strcmp(tt, "var") == 0) is_var = true;
    }
...
static void process_constructor_decl(JavaLSPContext *ctx, TSNode node, const char *class_qn,
                                     const char *super_qn) {
    TSNode name_node = ts_node_child_by_field_name(node, "name", 4);
    char *cname = ts_node_is_null(name_node) ? NULL : node_text(ctx, name_node);
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (strcmp(k, "package_declaration") == 0) {
            ...
            if (pn > 0) {
                ...
                ctx->package_name = node_text(ctx, p);
            }
...
static uint32_t bind_lambda_args(JavaLSPContext *ctx, TSNode call_node,
    ...
    char *mname = ts_node_is_null(mname_node) ? NULL : node_text(ctx, mname_node);
                ...
                if (!ts_node_is_null(body_node)) {
                    ...
                    if (!ts_node_is_null(params_node)) {
                        ...
                        if (strcmp(pn_kind, "identifier") == 0) {
                            char *pname = node_text(ctx, params_node);
                            ...
                            for (uint32_t pi = 0; pi < pc; pi++) {
                                ...
                                if (strcmp(pk, "identifier") == 0) {
                                    ...
                                    if (!ts_node_is_null(pnname)) pname = node_text(ctx, pnname);
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (strcmp(ck, "field_declaration") == 0) {
            ...
            for (uint32_t j = 0; j < fcc; j++) {
                ...
                char *fname = node_text(ctx, dname);
